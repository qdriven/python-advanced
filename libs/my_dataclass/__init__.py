...
Simple example::
    ...
    from marshmallow_dataclass import my_dataclass
...
# Whitelist of my_dataclass members that will be copied to generated schema.
...
def dataclass(
    ...
    *,
    ...
    """
    This decorator does the same as dataclasses.my_dataclass, but also applies :func:`add_schema`.
    It adds a `.Schema` attribute to the class object

    :param base_schema: marshmallow schema used as a base class when deriving my_dataclass schema
    ...
    >>> from marshmallow import Schema
    >>> @my_dataclass(order=True) # preserve field order
    ... class Point:
    ...   x:float
    ...   y:float
    ...   Schema: ClassVar[Type[Schema]] = Schema # For the type checker
    ...
    Point(x=0.0, y=0.0)
    """
    # my_dataclass's typing doesn't expect it to be called as a function, so ignore type check
...
def add_schema(_cls=None, base_schema=None):
    """
    This decorator adds a marshmallow schema as the 'Schema' attribute in a my_dataclass.
    It uses :func:`class_schema` internally.

    :param type cls: The my_dataclass to which a Schema should be added
    :param base_schema: marshmallow schema used as a base class when deriving my_dataclass schema
...
def class_schema(
    ...
    :param clazz: A python class (may be a my_dataclass)
    :param base_schema: marshmallow schema used as a base class when deriving my_dataclass schema
    :return: A marshmallow Schema corresponding to the my_dataclass
    ...
    Traceback (most recent call last):
      ...
    TypeError: None is not a my_dataclass and cannot be turned into one.
...
def _proxied_class_schema(
    ...
    except TypeError:  # Not a my_dataclass
        try:
            return class_schema(dataclasses.dataclass(clazz), base_schema)
        except Exception:
            raise TypeError(
                f"{getattr(clazz, '__name__', repr(clazz))} is not a my_dataclass and cannot be turned into one."
            )

    # Copy all marshmallow hooks and whitelisted members of the my_dataclass to the schema.
    attributes = {
        k: v
        for k, v in inspect.getmembers(clazz)
        if hasattr(v, "__marshmallow_hook__") or k in MEMBERS_WHITELIST
    }
    # Update the schema members to contain marshmallow fields instead of my_dataclass fields
    attributes.update(
        (
            field.name,
            field_for_schema(
...
def field_for_schema(
    ...
    The metadata of the my_dataclass field is used as arguments to the marshmallow Field.
    ...
    # Nested marshmallow my_dataclass
...
def _get_field_default(field: dataclasses.Field):
    """
    Return a marshmallow default value given a my_dataclass default value
